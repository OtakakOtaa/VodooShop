...
namespace CodeBase.Editor.OriginGameConfig
{
    public sealed class GameConfigMapper
    {
        ...
        private SyncDictionary<int, PlotCustomer> FindOnlyPlotCustomers()
            => MapCustomers()
                .OfType<PlotCustomer>()
                .ToSyncDictionary(k => GetDayNumberByPlotId(k.Id), v => v);
        ...
        private IEnumerable<Customer> MapCustomers()
            => _container.Customers
                .Select(c =>
                {
                    ...
                    return isPlot is false ? 
                        new Customer(c.Id, c.Name) : 
                        new PlotCustomer(c.Id, c.Name, ownedOrders);
                });

        private IEnumerable<CustomerOrder> FindOrdersWithoutOwner()
            => _container.Orders
                .Where(o => o.CustomerId == string.Empty)
        ...
        private int GetDayNumberByPlotId(string id)
            => CastWithoutNullState(_container.Days.First(d => d.StoryCustomersId.Contains(id)).LevelId);
